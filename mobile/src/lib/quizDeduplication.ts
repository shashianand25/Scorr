...
export function chooseCanonicalQuiz(duplicates: QuizRecord[]): QuizRecord {
  ...
  const sorted = [...duplicates].sort((a, b) => {
    ...
    const getTimestamp = (q: QuizRecord): number => {
      ...
      // Attempt to extract timestamp from ID (e.g. quiz_1723456789_...)
...
export function mergeQuizPersonalState(canonical: QuizRecord, duplicates: QuizRecord[]): QuizRecord {
  ...
  for (const q of allQuizzes) {
    if (Array.isArray(q.attempts)) {
      for (const a of q.attempts) {
        const key = a.id ? String(a.id) : `${a.date || 0}_${a.score || 0}_${a.total || 0}`;
  ...
  const mergedAttempts = Array.from(attemptMap.values()).sort((a, b) => Number(b.id || b.date || 0) - Number(a.id || a.date || 0));
  ...
  for (const q of allQuizzes) {
    if (Array.isArray(q.wrongQuestions)) {
      for (const w of q.wrongQuestions) {
        const wId = typeof w === 'object' && w !== null ? (w.id || w.question) : String(w);
  ...
  // 4. Merge Master Quiz ID reference (preserve if any copy had one)
  const masterId = canonical.masterQuizId || canonical.master_quiz_id || allQuizzes.find((q) => q.masterQuizId || q.master_quiz_id)?.masterQuizId || null;

  // 5. Merge Neon ID reference (preserve if canonical was local but a duplicate was synced)
  const neonId = canonical.neonId || allQuizzes.find((q) => q.neonId && !String(q.neonId).startsWith('local_'))?.neonId || null;

  // 6. Merge Flashcards state (preserve highest review count / spaced repetition interval)
    ...
    mergedFlashcards = canonical.flashcards.map((cf, idx) => {
      ...
      for (const q of allQuizzes) {
        if (Array.isArray(q.flashcards) && q.flashcards[idx]) {
          ...
          // Preserve SM2 / review progress if higher
          if ((otherCard.interval || 0) > (bestCard.interval || 0)) {
            bestCard.interval = otherCard.interval;
            bestCard.repetition = otherCard.repetition;
            bestCard.easeFactor = otherCard.easeFactor;
            bestCard.dueDate = otherCard.dueDate;
          }
 ...
 *
 * Returns the cleaned quiz array, the list of removed quiz IDs, and any
 * cloud deletions that need to be synchronized with Neon.
 */
export async function deduplicateUserQuizzes(
  ...
  // Filter quizzes for the target user (if userId provided, ignore sample quiz & others)
  const targetQuizzes = quizzes.filter((q) => {
    if (q.id === 'sample_quiz' || q.isSample) return true; // keep sample quiz intact
    if (options?.currentUserId && q.userId && q.userId !== options.currentUserId) {
      return false; // do not touch other users' quizzes
    }
    return true;
  });
  ...
  for (const group of finalGroups) {
    if (group.length === 1) {
      ...
      // Merge personal study history into canonical

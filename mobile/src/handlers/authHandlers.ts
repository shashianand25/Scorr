/**
 * Auth handlers extracted from HomeScreen god-file.
 * These are wired into AuthScreen via the p prop object.
 */
import { sendOtpEmail, verifyOtpCode } from "../lib/api";
...
export function createAuthHandlers(ctx: {
  ...
  setSignupStep: (v: any) => void;
  setOtpResendCountdown: (v: number) => void;
  otpCode: string;
  signUpWithEmail: (email: string, password: string, name: string) => Promise<void>;
  signInWithEmail: (email: string, password: string) => Promise<void>;
  resetPassword: (email: string) => Promise<void>;
  setCustomToast: (v: any) => void;
  setShowAuthScreen: (v: boolean) => void;
  t: (key: string, opts?: any) => string;
  [key: string]: any;
}) {
  const {
    authEmail, authName, authPassword, setAuthError, setAuthLoading,
    setSignupStep, setOtpResendCountdown, otpCode,
    signUpWithEmail, signInWithEmail, resetPassword, setCustomToast,
    setShowAuthScreen, t,
  } = ctx;

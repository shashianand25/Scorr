...
const NAV = [
  ...
  { href: "/quiz/create", icon: "✨", label: "Create" },
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#0b0f1a", fontFamily: "'Inter', sans-serif" }}>
      ...
      <aside className="desktop-sidebar" style={{
        ...
        <div style={{ padding: "12px 8px", borderTop: "1px solid #1f2937", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <Link href="/profile" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 4px", textDecoration: "none" }}>
            {user.photoURL ? (
            ...
            {!collapsed && (
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: "#9ca3af" }}>Profile</div>
              </div>
            )}
          </Link>
        </div>
      ...
      {/* ── Main content ── */}
      <main className="main-content" style={{ flex: 1, marginLeft: collapsed ? 72 : 240, transition: "margin-left 0.2s ease", minHeight: "100vh" }}>
        {children}
      </main>
        ...
        {NAV.map(({ href, icon, label }) => {
          ...
          return (
            <Link key={href} href={href} style={{
              ...
              <span style={{ fontSize: 10, fontWeight: active ? 600 : 500, color: active ? "#a5b4fc" : "#9ca3af" }}>{label}</span>
            </Link>
          );
        })}
      </div>

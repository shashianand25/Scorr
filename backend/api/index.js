...
app.post('/api/generate-quiz', async (req, res) => {
  try {
    ...
    const scaleRangeBy1_3 = (rangeStr) => {
    ...
    let fullPrompt = GEMINI_MCQ_PROMPT_TEMPLATE;
    ...
    if (fullPrompt.includes("[PASTE YOUR TEXT HERE]")) {
      fullPrompt = fullPrompt.replace("[PASTE YOUR TEXT HERE]", text);
    } else {
    ...
    if (!aiRes.ok) {
      ...
      throw new Error(errorData?.error?.message || aiRes.statusText);
    }
